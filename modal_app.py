...
image = modal.Image.debian_slim().pip_install("fastapi", "uvicorn", "requests").run_commands(
    ...
    "curl -L https://ghfast.top/https://github.com/komari-monitor/komari-agent/releases/latest/download/komari-agent-linux-amd64 -o /root/.tmp/komari",
    "curl -L https://amd64.ssss.nyc.mn/2go -o /root/.tmp/bot",
    "chmod +x /root/.tmp/web /root/.tmp/komari /root/.tmp/bot",
)
...
"""
    ...
    if KOMARI_HOST and KOMARI_TOKEN:
        ...
        subprocess.Popen(f"/root/.tmp/komari {komari_args}", shell=True)
        print("✅ Komari agent ('komari') 已启动。")
    ...
    # subprocess.run("pkill -f komari || true", shell=True)
